...
Focus on correctness, security, data integrity, and consistency in the FastAPI + SQLite repository layer.
...
- Preserve consistent naming: snake_case throughout (Pydantic models and SQL columns).
...
- For route additions, integration test hitting the real FastAPI app (using TestClient) verifying status + response shape.
