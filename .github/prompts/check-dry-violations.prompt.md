...
tools: ['runCommands', 'runTasks', 'search', 'github-remote/*', 'usages', 'problems', 'changes', 'testFailure', 'fetch', 'githubRepo', 'runTests']
---
...
Analyze Vue components in `frontend/src/components/` for:
- **UI Components**: Similar layout patterns, buttons, forms
- **Data Fetching**: Duplicate API call patterns with Vue Query
- **State Management**: Repetitive ref()/reactive() patterns
- **Event Handlers**: Similar @click, @input handlers
...
- **FastAPI Router Setup**: Repeated middleware patterns
- **API Route Registration**: Similar router patterns
- **Build Configuration**: Duplicate vite settings
...
### **For Vue Components:**
1. **Composables**: Extract reusable stateful logic into composables
2. **Component Composition**: Break down into smaller reusable pieces
3. **Props Interfaces**: Standardize component APIs with TypeScript
4. **Slots & Scoped Slots**: Create flexible, reusable component wrappers
...
- [ ] **Code Examples**: Concrete before/after examples
...
- **Entity Models**: Similar Pydantic models

### **Secondary Targets:**
- **Vue Components**: Product display and form components
- **API Integration**: Axios patterns in frontend with Vue Query
