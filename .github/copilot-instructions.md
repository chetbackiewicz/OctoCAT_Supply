...
- `api/` FastAPI REST API (Python, SQLite persistence, repository pattern, Swagger docs)
- `frontend/` Vue 3 + Vite + Tailwind UI (Composition API, Pinia state management, Vue Router)
- Shared docs under `docs/` and deployment scripts under `infra/`
...
3. Encourage type safety (use proper type hints, avoid `Any` unless justified). Suggest adding/refining Pydantic models or type annotations when gaps appear.
...
6. Encourage tests: request unit tests for new repository logic and component tests for critical UI paths.
