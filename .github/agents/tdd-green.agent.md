...
tools: ['runTasks', 'edit', 'search', 'usages', 'problems', 'testFailure', 'githubRepo', 'todos', 'runSubagent', 'runTests']
handoffs:
...
class ThingRepository:
    async def get_by_id(self, id: int) -> Thing | None:
        # Use existing database patterns
        row = await db.get("SELECT ...")
        return map_row_to_thing(row) if row else None
```
...
```python
@router.get("/{id}")
async def get_thing(id: int, repository: ThingRepository = Depends()):
    thing = await repository.get_by_id(id)
    if not thing:
