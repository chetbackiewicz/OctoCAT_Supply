...
tools: ['edit', 'search', 'usages', 'problems', 'fetch', 'githubRepo', 'todos', 'runSubagent']
handoffs:
...
- [ ] `{test_file.py}` - Test file (to be created by Red agent)
- [ ] `{implementation_file.py}` - Implementation (to be created by Green agent)
- [ ] `{related_file.py}` - Supporting changes if needed
